...
def log_interp1d(x, y,
    ...
    def log_interp(z): return np.exp(lin_interp(np.log(z)))
    return log_interp


def logx_InterpolatedUnivariateSpline(x, y, **kwargs):
    from scipy.interpolate import InterpolatedUnivariateSpline as IUS
    log_IUS = IUS(np.log(x), y, **kwargs)
    return lambda x: log_IUS(np.log(x))


def logy_InterpolatedUnivariateSpline(x, y, **kwargs):
    from scipy.interpolate import InterpolatedUnivariateSpline as IUS
    log_IUS = IUS(x, np.log(y), **kwargs)
    return lambda x: np.exp(log_IUS(x))
