...
def backward_propagation_three_layer(dL, x0, x1, x2, w1, w2, w3, dact=d_LU):
    ...
    # dL/dw2; note element-wise product with the activation derivative
    grad_w2 = x1.T.dot(grad_x2*dact(x2))
    grad_x1 = grad_x2.dot(w2.T)          # dL/dx1
    # dL/dw1; note element-wise product with the activation derivative
    grad_w1 = x0.T.dot(grad_x1*dact(x1))
...
def backward_propagation(dL, xs, ws, dact=d_LU):
    ...
    for i in range(n):
        if i == 0:
            # First neuron gradient is dL/dx[-1]
            grad_x = dL
            # First weight gradient has no non-linearity
            grad_w = xs[-2-i].T.dot(grad_x)
        else:
            # dL/dx are all vectors
            grad_x = grad_x.dot(ws[0-i].T)
            # dL/dw are matrices
            grad_w = xs[-2-i].T.dot(grad_x*dact(xs[-1-i]))
...
def backward_timestep_lag_corrected(i, x0, x1, x2, g0, g1, g2, W0, W1, l01, l12, dact):
    ...
    g0[i] = matmul((g1[i-l01]+(g1[i-l01]-g1[i-l01-1])*l01)
                   * dact(x1[i-2*(l01+l12)]), W0[i])
...
def AdaGrad(grad, x, g2_sum, alpha=1e-3, eps=1e-7, **kwargs):
    grad = grad(x, **kwargs)
    # Accumulate sum of gradient-squares in each direction; returns a vector
    g2_sumn = g2_sum+grad**2
...
def RMSProp(grad, x, g2, alpha=1e-3, rho=0.99, eps=1e-7, **kwargs):
    grad = grad(x, **kwargs)
    # Accumulates a sum of gradient-squares that decays; returns a vector
    g2n = rho*g2+(1.-rho)*grad**2
...
def Adam(grad, x, m1, m2, t, alpha=1e-3, beta1=0.99, beta2=0.99, eps=1e-7, unbiasing=True, **kwargs):
    ...
    '''
    grad = grad(x, **kwargs)
    # Sort of like velocity/momentum/friction part
