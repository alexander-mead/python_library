...
def enumerate_actions(maze):
    ...
    for (ir, ic), tile in np.ndenumerate(maze):
        ...
        if tile == maze_def.index('wall'):
            ...
            if maze[ir, iR] != maze_def.index('wall'):
                tile_actions.append(action_right)
            if maze[ir, iL] != maze_def.index('wall'):
                tile_actions.append(action_left)
            if maze[iU, ic] != maze_def.index('wall'):
                tile_actions.append(action_up)
            if maze[iD, ic] != maze_def.index('wall'):
                tile_actions.append(action_down)
    ...
    for _ in range(num_attempts):
        ...
        for _ in range(max_steps):
            ...
            if action == action_exit:
                break
            r, c = _move(r, c, nr, nc, action)
        attempts_r.append(rs)
        attempts_c.append(cs)
    payoffs = []
...
def learn_best_policy(maze, actions, rewards, verbose=False, gamma=gamma_def, rseed=None, method='policy_iteration'):
    ...
    if verbose:
        mead.print_full_array(state_map, 'State map:')
    print()
    if verbose:
        mead.print_full_array(rewards_map, 'Rewards:')
    print()
    n = len(state_map)
    if verbose:
    ...
    for i in range(n):  # Loop over states
        l = state_map.index((state_map[i][0], mead.periodic_integer(
            state_map[i][1]-1, nc))) if action_left in actions[state_map[i]] else i
        r = state_map.index((state_map[i][0], mead.periodic_integer(
            state_map[i][1]+1, nc))) if action_right in actions[state_map[i]] else i
        u = state_map.index((mead.periodic_integer(
            state_map[i][0]+1, nr), state_map[i][1])) if action_up in actions[state_map[i]] else i
        d = state_map.index((mead.periodic_integer(
            state_map[i][0]-1, nr), state_map[i][1])) if action_down in actions[state_map[i]] else i
        states_left.append(l)
        states_right.append(r)
        states_up.append(u)
        states_down.append(d)
    ...
    if verbose:
        print('Example transition matrix for left:\n',
