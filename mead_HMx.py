...
def fitted_power_file_name(mode, model, z, chain, field_pair, label):
    ...
    file = dir+'/m'+str(mode)+'/'+model + \
        '/c%d_z%1.3f_cos1_%s_power_%d%d.dat' % (chain, z, label, f1, f2)
    return file
...
def get_fitted_response(mode, model, z, chain, field_pair, label):
    ...
    _, dmonly = get_fitted_power(
        mode, model, z, chain, ('dmonly', 'dmonly'), label)
...
def get_HMcode_power(mode, model, z, chain):
    ...
    dir = '/Users/Mead/Physics/HMx/fitting'
    infile = dir+'/m'+str(mode)+'/'+model + \
        '/c1_z%1.3f_cos1_HMcode_power.dat' % (z)
...
def parameter_file_name(fit, model, chain, z=None):
    ...
    if (z == None):
        ...
        file = directory + \
            'fitting/m%d/%s/c%d_z%1.3f_params.dat' % (fit, model, chain, z)
...
def read_parameter_file(file):
    ...
    figure_of_merit = read_first_number_from_line_in_file(
        file, parameter_file_figure_of_merit_line)
...
def read_parameter_file_models_zs(fit, models, chain, zs):
    ...
    for model in models:
        ...
        for z in zs:
            ...
            # Read the data
            parameter_names, parameter_values, figure_of_merit = read_parameter_file(
                infile)
    ...
    best_fitting_values_models = np.asarray(best_fitting_values_models)
    figures_of_merit = np.asarray(figures_of_merit)
