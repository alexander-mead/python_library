...
def read_gadget(snapshot, ptype=[1]):
    ...
    pos = readgadget.read_block(
        snapshot, "POS ", ptype)/1e3  # Positions in [Mpc/h]
    # Peculiar velocities in [km/s]
    vel = readgadget.read_block(snapshot, "VEL ", ptype)
    ids = readgadget.read_block(
        snapshot, "ID  ", ptype)-1   # IDs starting from 0

    # Print more information
    for i, name in zip([0, 1, 2], ['x', 'y', 'z']):
        print('Min/max position [Mpc/h]:', name,
              pos[:, i].min(), pos[:, i].max())
    print()
...
def read_fof(snapdir, snapnum):
    ...
    FoF = readfof.FoF_catalog(
        snapdir, snapnum, long_ids=False, swap=False, SFR=False, read_IDs=False)
