        ...
        for ic, param_c in enumerate(params):
            ...
            if ir == ic:
                ...
                if truths is not None:
                    plt.axvline(truths[param_r], color='black', ls='--', alpha=0.7, label='Truth')
                ...
                plt.xlabel(labels[param_r]) if ic==n-1 else plt.gca().set_xticklabels([])
                plt.yticks([])
                if (truths is not None) and iplot == 1: plt.legend(loc='upper left', bbox_to_anchor=(1., 1.))
            elif ir > ic:
                plt.subplot(n, n, iplot)
                if truths is not None:
                    plt.plot([truths[param_c]], [truths[param_r]], color='black', marker='x', alpha=0.7, label='Truth')
                for dict_of_samples in dicts_of_samples:
                    plt.scatter(dict_of_samples[param_c], dict_of_samples[param_r], 
                            alpha=scatter_alpha, s=scatter_size,
                    )
                plt.xlabel(labels[param_c]) if ir==n-1 else plt.gca().set_xticklabels([])
                plt.ylabel(labels[param_r]) if ic==0 else plt.gca().set_yticklabels([])
