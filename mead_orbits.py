...
def _mean_anomaly(nu, e):
    def integrand(x): return 1./(1.+e*np.cos(x))**2
...
class orbit():
    ...
    def __init__(self, Mass=1., a=1., e=0., omega=0., nu=None, m=None, G=4.*np.pi**2):
        ...
        if (nu is None) and (m is None):
            raise ValueError(
                'You must specify either mean anomaly or true anomaly.')
        elif nu is None:
            self.m = np.deg2rad(m)
            ...
            raise ValueError(
                'Cannot specify both mean anomaly and true anomaly')
    ...
    def true_anomaly(self, approximate=False):
        ...
        e = self.e
        if approximate:
            # Approximate numerical inversion for small e
            nu = m+2.*e*np.sin(m)
        else:
            def f(x): return _mean_anomaly(x, e)-m
            # nu_guess = m # This is probably a pretty good first guess
            # Use approximate (e<<1) solution as first guess
            nu_guess = self.true_anomaly(approximate=True)
    ...
    def _time(self, theta):
        ...
        integral, _ = quad(lambda x: integrand(
            x, e, omega), a=omega+nu, b=theta)
        time = P*fac*integral
        return time
