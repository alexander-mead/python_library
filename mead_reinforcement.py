...
def calculate_policy_values(policy, transitions, rewards, gamma=gamma_def):
    ...
    V = np.linalg.solve(np.identity(n)-gamma*prob,
                        rewards)  # Linear equation solving
    return V
...
def _value_iteration(transitions, rewards, gamma=gamma_def, verbose=False):
    ...
    while not np.array_equal(old_values, new_values):
        ...
        for state in range(number_of_states):
            ...
            for action in actions:  # Find the expected future rewards associated with all possible actions
                expected_future_rewards[action] = np.dot(
                    transitions[action][state, :], old_values)
            ...
            new_values[state] = rewards[state]+gamma * \
                max(expected_future_rewards.values())
    if verbose:
        print('Best values found in stpes:', steps)
    ...
    for state in range(number_of_states):
        ...
        for action in actions:
            expected_future_rewards[action] = np.dot(
                transitions[action][state, :], best_values)
        # Action with best reward
        policy_item = max(expected_future_rewards,
                          key=expected_future_rewards.get)
...
def _policy_iteration(transitions, rewards, gamma=gamma_def, rseed=None, verbose=False):
    ...
    while new_policy != old_policy:
        ...
        values = calculate_policy_values(
            old_policy, transitions, rewards, gamma)
        expected_future_rewards = {}
        for action in actions:
...
def compute_best_policy(transitions, rewards, gamma=gamma_def, method='policy_iteration', rseed=None, verbose=False):
    ...
    if method == 'policy_iteration':
        best_policy = _policy_iteration(
            transitions, rewards, gamma=gamma, rseed=rseed, verbose=verbose)
    elif method == 'value_iteration':
        best_policy = _value_iteration(
            transitions, rewards, gamma=gamma, verbose=verbose)
    else:
        print('Method:', method)
