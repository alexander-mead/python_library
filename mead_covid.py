...
# General
# Number of days that contribute to a 'roll' (one week; seven days)
days_in_roll = 7
# Normalisation for y axes (per population; usually 100,000; sometimes 1,000,000)
pop_norm_num = 1e5
# Number of days an average person is infectious for (used in calculations of R number; lower values squash R about 1)
infect_duration = 5.
...
def sort_month_axis(plt):
    ...
    locator_monthmid = dates.MonthLocator(
        bymonthday=15)  # Middle of every month
...
def plot_lockdown_spans(plt, data, region, color='red', alpha=0.25, label='Lockdown'):
    ...
    for id, dates in enumerate(lockdowns.get(region)):
        ...
        if len(dates) == 1:
            ...
            raise TypeError(
                'Lockdown dates should have either one or two entries')
        if id == 0:
            llabel = label
        else:
            llabel = None
...
def read_Canada_data(infile):
    ...
    df.sort_values(by=['prname', 'date'], ascending=[
...
def read_Canada_vaccine_data(infile):
    df = pd.read_csv(infile)
    df.rename(columns={'prename': 'prname', 'week_end': 'date'},
    ...
    # Some columns have '<0.01' in them which means column is read as string
    df.replace('<0.01', '0.00', inplace=True)
    df['proptotal_fully'] = df['proptotal_fully'].astype(
        float)  # Convert the string column to a float
    return df
...
def plot_Canada_data(df, df_hosp, start_date, end_date, death_fac=20, Nmax=None):
    ...
    # Bars
    dfs = [df[df['prname'] == province], df_hosp,
           df_hosp, df[df['prname'] == province]]
    ...
    ax2 = ax.secondary_yaxis('right', functions=(
        lambda y: y/y2fac, lambda y: y*y2fac))
    ...
    plt.title(title, x=titx, y=tity, loc='left', fontsize='small',
...
def plot_Canadian_provinces_data(df, provinces, start_date, end_date, Nmax=100):
    ...
    for iplot, province in enumerate(provinces):
        ...
        facs = [pop_norm_num/pop[province],
                death_fac*pop_norm_num/pop[province]]
        ...
        for thing, fac, color, label in zip(things, facs, colors, labels):
            plt.plot(
                df[df['prname'] == province]['date'] -
                dt.timedelta(rolling_offset),
                df[df['prname'] == province][thing]*fac,
                color=color,
                label=label,
            )
...
def plot_Canadian_province_data(df, start_date, end_date,
    ...
    plot_country_cases = try_to_plot_country_cases and (
        df_world_cases is not None)
    plot_country_deaths = try_to_plot_country_deaths and (
        df_world_deaths is not None)
    ...
    if province == 'British Columbia':
        ...
        for label, special_date, dir, ypos in zip(special_dates.keys(), special_dates.values(), dirs, yposes):
            ...
            if dir == 'left':
                plt.text(special_date+relativedelta(days=-2), ypos*Nmax, label +
                         ' →', size='small', ha='right')  # , transform=ax.transAxes)
            else:
                plt.text(special_date+relativedelta(days=+2), ypos*Nmax, '← ' +
                         label, size='small', ha='left')  # , transform=ax.transAxes)
        ...
        if plot_restrictions:
            restrictions = {
                'school closures': {
                    # Initial March 2020 school closure
                    (date(2020, 3, 15), date(2020, 6, 1)),
                },
                'mask mandate': {
                    # Masks first madated indoors in November 2020
                    (date(2020, 11, 19), date(2021, 7, 1)),
                    # Brief period in July/August 2021 where mask mandate was dropped
                    (date(2021, 8, 25), date(2022, 3, 11)),
                },
                'gym closures': {
                    # Initial March 2020 gym closures
                    (date(2020, 3, 21), date(2020, 5, 19)),
                    # Step 1 of restart BC (TODO: Start date)
                    (date(2020, 10, 19), date(2021, 5, 25)),
                    # Omicron gym closures
                    (date(2021, 12, 22), date(2022, 1, 18)),
                },
                'indoor gatherings limited': {
                    # Step 2 of restart BC
                    (date(2020, 10, 19), date(2021, 6, 15)),
                    # Gatherings limited  because of Omicron
                    (date(2021, 12, 22), date(2022, 2, 16)),
                },
                'indoor gatherings banned': {
                    # Winter 2020 firebreak lockdown
                    (date(2020, 11, 8), date(2020, 11, 23)),
                    # Step 1 of restart BC (TODO: Start date)
                    (date(2020, 10, 19), date(2021, 5, 25)),
                },
                'vaccine passport': {
                    # Vaccine passports introduced September 2021
                    (date(2021, 9, 13), date(2022, 4, 8)),
                }
            }
            add_label = True
        ...
        if plot_country_cases:
            ...
            fac = population_Canadian_provinces[province] / \
                population_countries[country]
        ...
        if plot_country_deaths:
            fac = population_Canadian_provinces[province] / \
                population_countries[country]*death_fac
            if country == 'United Kingdom':
                label = 'UK deaths'
            else:
        ...
        if plot_vax and df_vax is not None:
            plt.axvspan(start_date, end_date, ymin=thr_dose_min,
                        ymax=one_dose_max, color='w', alpha=1., lw=0)
            ax.plot([0., 1.], [restriction_max, restriction_max],
                    transform=ax.transAxes, lw=0.5, color='black')
            ...
            for ypos, label in zip(yposs, labels):
                plt.text(0.89, ypos, label, va='bottom',
                         size='x-small', transform=ax.transAxes)
...
def read_JHU_data(file):
    ...
    '''
    df = pd.read_csv(file)
    df.drop(['Province/State', 'Lat', 'Long'], axis='columns',
            inplace=True)  # Remove unnecessary columns
    df.rename(columns={'Country/Region': 'Region'},
              inplace=True)  # Rename for convenience
    # Combine provinces that are part of the same countries
    df = df.groupby(['Region']).sum()
    # Convert entries to datetime objects
...
def calculate_JHU_daily_from_original(df):
    ...
    '''
    df_new = df.diff(axis='columns').copy(
    )  # Convert from cumulative to daily numbers
    return df_new
...
def calculate_JHU_rolling_from_daily(df):
    ...
    '''
    center = True
    df_new = df.rolling(7, min_periods=1, axis='columns',
                        center=center).sum()/7.  # Average over week
...
def plot_UK_deaths(df_daily, df_rolling):
    ...
    deaths = {  # Comparable deaths
        # 2019 total averaged over 365 days
        'All-cause deaths in a typical year': 530841./365.,
        'Flu deaths in a typical flu year': 15000./365.,  # Total averaged over year
        # Compress total over 3 months (shoddy)
        'Flu deaths at height of a bad flu year': 4.*25000./365.,
    }
    ...
    sns.lineplot(data=df_rolling.loc[country],
                 label='UK COVID-19 deaths', color=death_line_color)
    plt.bar(df_daily.loc[country].index, df_daily.loc[country].values,
            width=1., color=death_bar_color, linewidth=0., alpha=bar_alpha)
    ...
    for i, death in enumerate(deaths.keys()):
        ...
        plt.text(max(df_rolling.columns)+relativedelta(months=1),
                 deaths[death]+delta, death)
    handles, _ = ax.get_legend_handles_labels()
    patch = mpatches.Patch(
        alpha=0., label=dt.date.today().strftime("%Y-%m-%d"))
...
def plot_world(df, countries, start_date, end_date, dftype='deaths'):
    ...
    for iplot in mead.mrange(1, 2):
        ...
        for country in countries:
            sns.lineplot(data=dg.loc[country]*per /
                         population_countries[country], label=country)
        plt.ylabel(ylab)
        plt.xlabel(None)
        plt.ylim(bottom=0.)
        plt.xlim((start_date, end_date))
...
def plot_world_deaths_per_case(df_deaths, df_cases, countries):
    ...
    plt.title(dt.date.today().strftime("%Y-%m-%d"), x=0.015, y=0.88,
              loc='Left', bbox=dict(facecolor='w', edgecolor='k'))
    plt.tight_layout()
...
def download_data(area, metrics, verbose=True):
    ...
    file = '/Users/Mead/COVID-19/govUK/data/' + \
        area+'_'+today.strftime("%Y-%m-%d")+'.csv'
...
def data_calculations(df, verbose):
    ...
    for col in ['Cases', 'Hosp', 'Beds', 'Deaths']:
        if col in df:
            ...
            df[col+'_roll_Mead'] = df.groupby('Region')[col].rolling(
                days_roll).sum(skipna=False).reset_index(0, drop=True)
    ...
    for col in ['Cases', 'Deaths']:
        if col+'_roll_Mead' in df:
            ...
            df[col+'_roll_past'] = df.groupby('Region')[col +
                                                        '_roll_Mead'].shift(periods=days_roll)
            ...
            df.drop([col+'_roll_past', col+'_ratio'],
    ...
    for region in regions:
        ...
        for col in ['Cases', 'Deaths']:
            if col in df:
                ...
                print('Daily new '+col.lower() +
                      ': %d, or per 100,000 population: %.1f.' % (daily, norm_daily))
        ...
        for col in ['Cases', 'Deaths']:
            if col+'_roll' in df:
                ...
                if col+'_roll_Mead' in df:
                    ...
                    if weekly != my_weekly:
                        raise ValueError(
                            'My calculation of weekly roll disagrees with official')
                norm_weekly = weekly*fac
                print('Weekly '+col.lower() +
                      ': %d, or per 100,000 population: %.1f.' % (weekly, norm_weekly))
...
def plot_bar_data(df, start_date, end_date, regions,
    ...
    # Initialise plot
    _, axs = plt.subplots(figsize=(figx, figy),
    ...
    for i, region in enumerate(regions):
        ...
        if plot_type in ['Long', 'Elongated']:
            ...
            if not mead.is_perfect_square(n):
                raise ValueError(
                    'Need a square number of plots for square plot')
            r = int(np.sqrt(n))
            plt.subplot(r, r, i+1)
        else:
        ...
        if plot_months:
            plot_month_spans(plt)
        if plot_lockdowns:
            plot_lockdown_spans(plt, df, region)
        ...
        if region == 'United Kingdom' and pop_norm == False:
            for i, death in enumerate(deaths.keys()):
                delta = 50.
                plt.axhline(deaths[death]*death_fac,
                            color='black', alpha=0.8, ls=':')
                plt.text(end_date+relativedelta(days=15),
                         (deaths[death]+delta)*death_fac, death)
        ...
        if pop_norm:
            if y2axis:
                raise ValueError(
                    'You need to code up y2 if pop_norm is active')
            if ylabel is None:
                ylabel = 'Number per day per 100,000 population'
        else:
            if y2axis:
                if ylabel is None:
                    ylabel = 'New cases per day'
                if y2label is None:
                    y2label = 'Deaths/Hospitalisations per day'
            else:
                if ylabel is None:
        ...
        if loc == 'right':
            ...
            xtit = dx
        plt.title(title, x=xtit, y=ytit, loc=loc,
                  bbox=dict(facecolor='w', edgecolor='k'))
        ...
        if (n == 4 or n == 9) and i == 0:
            ...
            if n == 4:
                ...
                ytit = 0.89
            plt.title(title_date, x=xtit, y=ytit, loc=legend_location,
                      bbox=dict(facecolor='w', edgecolor='k'))
        ...
        if y2axis:
            y2fac = death_fac
            ax2 = axs.secondary_yaxis('right', functions=(
                lambda y: y/y2fac, lambda y: y*y2fac))
            ax2.set_ylabel(y2label)
            ax2.get_yaxis().set_major_formatter(
                ticker.FuncFormatter(lambda x, _: format(int(x), ',')))
...
def plot_rolling_data(df, start_date, end_date, regions, pop_norm=True, plot_type='Cases', log=True):
    ...
    if plot_months:
        plot_month_spans(plt)
    if plot_lockdowns:
        plot_lockdown_spans(plt, df, lockdown_region)
    ...
    for i, region in enumerate(regions):
        ...
        plt.title('Daily new '+plot_type.lower() +
                  ': %s' % (title_date.strftime("%Y-%m-%d")))
...
def plot_doubling_times(df, start_date, end_date, regions, plot_type='Cases'):
    ...
    for i, region in enumerate(regions):
        ...
        plt.title(plot_type+' doubling or halving time: %s' %
                  (title_date.strftime("%Y-%m-%d")))
